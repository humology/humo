defmodule Mix.Tasks.Humo.Config.GenTest do
  use ExUnit.Case

  import MixHelper

  alias Mix.Tasks.Humo.Config.Gen
  ...
  test "create configs based on mix project files" do
    in_tmp("mix_humo_npm_install", fn ->
      ...
      assert_received {:mix_shell, :info, ["Running task humo.config.gen"]}
      assert_received {:mix_shell, :info, ["* creating config/test_deps.exs"]}
      assert_received {:mix_shell, :info, ["* creating config/dev_deps.exs"]}
      assert_received {:mix_shell, :info, ["* creating config/prod_deps.exs"]}
      ...
      assert_file "config/test_deps.exs", expected_config
      assert_file "config/dev_deps.exs", expected_config
      assert_file "config/prod_deps.exs", expected_config
    end)
  end
    ...
    for only <- [:test, :dev, :prod, [:test, :dev], [:dev, :prod]] do
      in_tmp("mix_humo_npm_install", fn ->
        ...
        for env <- [:test, :dev, :prod] do
          file_content = File.read!("config/#{env}_deps.exs")
